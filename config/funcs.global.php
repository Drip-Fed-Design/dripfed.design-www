...
{
    return ($a['articlePublishDate'] <= $b['articlePublishDate']) ? 1 : -1;
}
...
// ------------------------------------------------------------
// ------------------------------------------------------------
// ---------- STRIP CONTENTFUL HTML FORMATTING FOR META ----------
// ----------
// ----------
//
function cleanArticleBody($articleMetaBody)
{
    // 1. Strip HTML tags
    $cleanedBody = strip_tags($articleMetaBody);

    // 2. Decode HTML entities
    $cleanedBody = html_entity_decode($cleanedBody, ENT_QUOTES | ENT_HTML5, 'UTF-8');
    ...
    $cleanedBody = strtr($cleanedBody, $smartQuotes);

    // 4. Replace double quotes with single quotes inside the text to avoid JSON conflicts
    $cleanedBody = str_replace('"', "'", $cleanedBody);

    // 5. Escape backslashes to prevent JSON errors
    $cleanedBody = str_replace('\\', '\\\\', $cleanedBody);

    // 6. Ensure periods are followed by a space if not already followed by a space or punctuation
    $cleanedBody = preg_replace('/\.(\S)/', '. $1', $cleanedBody);

    // 7. Remove extra white spaces and new lines
    $cleanedBody = preg_replace('/\s+/', ' ', $cleanedBody);
...
// ------------------------------------------------------------
// ------------------------------------------------------------
// ---------- FORMAT IMAGE TITLE FOR SEO ----------
// ----------
// ----------
//
function formatImageTitle($imgTitle)
{
    // 1. Remove the file extension
    $titleWithoutExtension = pathinfo($imgTitle, PATHINFO_FILENAME);

    // 2. Replace hyphens with spaces
    $titleWithSpaces = str_replace('-', ' ', $titleWithoutExtension);

    // 3. Capitalize the first letter of each word
    $seoFriendlyTitle = ucwords($titleWithSpaces);
